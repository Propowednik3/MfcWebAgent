...
#define IDT_HIDDEN			10
#define IDT_APPEARING		11
#define IDT_WAITING			12
#define IDT_DISAPPEARING	13
...
CTaskbarNotifier::~CTaskbarNotifier()
{
	...
	if (m_strCaption == NULL) free(m_strCaption);
	if (m_strBody == NULL) free(m_strBody);
	if (m_strLogo == NULL) free(m_strLogo);	
}
...
void CTaskbarNotifier::ExtraHide(int nIncrement)
{
...
void CTaskbarNotifier::Wait(DWORD dwTimeToLive)
{
	if (m_nAnimStatus == IDT_WAITING) 
	{
		m_changeTime = dwTimeToLive;
		if (dwTimeToLive == 0)
		{	
			OnTimer(IDT_WAITING);
		}
		m_changeTime = dwTimeToLive;
		//lPrint("Set time %i", m_dwTimeToLive);
	}
}

void CTaskbarNotifier::Show(char* szLogo, char* szCaption, char* szBody,
							DWORD dwTimeToShow,DWORD dwTimeToLive,DWORD dwTimeToHide,int nIncrement)
{
	...
	if (m_strLogo) {free(m_strLogo); m_strLogo = NULL;}
	if (m_strCaption) {free(m_strCaption); m_strCaption = NULL;}
	if (m_strBody) {free(m_strBody); m_strBody = NULL;}
	
	if (szLogo)
	{
		m_strLogo = (char*)malloc(strlen(szLogo) + 10);
		memset(m_strLogo, 0, strlen(szLogo) + 10);
		strcpy(m_strLogo, szLogo);
	}
	if (szCaption)
	{
		m_strCaption = (char*)malloc(strlen(szCaption) + 10);
		memset(m_strCaption, 0, strlen(szCaption) + 10);
		strcpy(m_strCaption, szCaption);
	}
	if (szBody)
	{
		m_strBody = (char*)malloc(strlen(szBody) + 10);
		memset(m_strBody, 0, strlen(szBody) + 10);
		strcpy(m_strBody, szBody);
	}
	...
	nDesktopWidth = nScreenWidth;
	nDesktopHeight = nScreenHeight;	
	...
	//nDesktopWidth = nScreenWidth;
	//nDesktopHeight = nScreenHeight;	
	//lPrint("%i %i %i %i %i %i",rcDesktop.bottom, rcDesktop.top, rcDesktop.left, rcDesktop.right, nScreenHeight, nScreenWidth);
	rcDesktop.bottom = nScreenHeight;
	rcDesktop.top = 0;
	rcDesktop.left = 0;
	rcDesktop.right = nScreenWidth;
...
{
	m_pWndParent->PostMessage(WM_NOTIFIER_LEFT_CLICKED,0,0);
	CWnd::OnLButtonUp(nFlags, point);
}

void CTaskbarNotifier::OnRButtonUp(UINT nFlags, CPoint point)
{
	m_pWndParent->PostMessage(WM_NOTIFIER_RIGHT_CLICKED,0,0);
	CWnd::OnRButtonUp(nFlags, point);
}

void CTaskbarNotifier::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	m_pWndParent->PostMessage(WM_NOTIFIER_DBL_LEFT_CLICKED,0,0);
...
void CTaskbarNotifier::OnPaint()
{
	...
	if (m_strCaption)
	{
		rcClient.SetRect(7,7,158,39);
		dc.DrawText(m_strCaption,rcClient,DT_LEFT | DT_TOP | DT_WORDBREAK | DT_END_ELLIPSIS);
	}

	if (m_strLogo)
	{
		rcClient.SetRect(7,39,158,52);
		dc.DrawText(m_strLogo,rcClient,DT_LEFT | DT_TOP | DT_WORDBREAK | DT_END_ELLIPSIS);
	}
	...
	if (pGdiImage == NULL)
	{
		if (m_strBody)
		{
			rcClient.SetRect(7,52,158,120);
			dc.DrawText(m_strBody,rcClient,DT_LEFT | DT_TOP | DT_WORDBREAK | DT_END_ELLIPSIS);
		}
	}	
	else
	{
		//rcClient.SetRect(7,37,158,47);
		//dc.DrawText(m_strBody,rcClient,DT_LEFT | DT_TOP | DT_WORDBREAK | DT_END_ELLIPSIS);
		using namespace Gdiplus;
		Image * m_picture = (Image*)pGdiImage;
		BlockSection.Lock();
		//CPaintDC dc(this);
		if ((m_picture) && (m_picture->GetLastStatus() == Ok))
		{
			Graphics graphics(dc.m_hDC);
			if(graphics.GetLastStatus() == Ok) 
				graphics.DrawImage(m_picture, 5, 52, 160, 120);		
		}
			...
			switch(m_nTaskbarPlacement)
			{
				case TASKBAR_ON_BOTTOM:
					...
					else
					{
						KillTimer(IDT_DISAPPEARING);
						Hide();
						m_pWndParent->PostMessage(WM_NOTIFIER_WINDOW_HIDED,0,0);
					}
					break;
				case TASKBAR_ON_TOP:
					if (m_nCurrentPosY>m_nStartPosY)
